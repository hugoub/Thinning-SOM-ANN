...
class SOM(object):
    ...
    def __init__(self, m, n, l, dim, n_iterations=100, alpha=None, sigma=None):
        """
        ...
        with self._graph.as_default():
            ...
            ## Vector de pesos
            self._weightage_vects = tf.Variable(tf.random_normal([m*n*l, dim]))
            
            ## Vector de localizacion de nodos
            self._location_vects = tf.constant(np.array(
                list(self._neuron_locations(m, n, l))))
            ...
            ## Es un arreglo de todas las distancias
            ...
            learning_rate_op_2 = tf.reshape(learning_rate_op, (m*n*l,1))
    ...
    def _neuron_locations(self, m, n, l):
        """
    ...
    def train(self, input_vects):
        ...
        centroid_grid = [[[] for i in range(self._n)] for i in range(self._m)]
        self._weightages = list(self._sess.run(self._weightage_vects))
        self._locations = list(self._sess.run(self._location_vects))
        for i, loc in enumerate(self._locations):
            centroid_grid[loc[0]][loc[1]].append(self._weightages[i])
    ...
    def map_vects(self, input_vects):
        ...
        locaciones = np.array(list(self._neuron_locations(self._m, self._n, self._l)))
        ...
        for vect in input_vects:
            ...
            if np.linalg.norm(locaciones[min_index] - locaciones[min_index_2]) > r3:
...
RUTA_DATASET = "~/ml_paper/datasets/madera" # Ruta del Workspace
...
for dim_m, dim_n, dim_l in dimensiones:
    
    for iteracion in num_iter:
        
        som = SOM(dim_m, dim_n, dim_l, cosecha_prepared.shape[1] , n_iterations = iteracion, alpha = l_rate)
        ...
        datos = open('./Resultados_SOM/resultadosSOM3D-'+str(l_rate)[2:]+'-'+str(+iteracion)+'-'+str(dim_m)+'x'+str(dim_n)+'x'+str(dim_l),"w")
        
        datos.write('centroides = '+str(centroides)+'\n')
        datos.write('mapped = '+str(mapped)+'\n')
        datos.write('m1 = '+str(dim_m)+'\n')
        datos.write('n1 = '+str(dim_n)+'\n')
        datos.write('l1 = '+str(dim_l)+'\n')
