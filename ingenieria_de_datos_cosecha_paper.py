...
for i in cosecha_dataframe['Unidad'].index:
    if cosecha_dataframe['Unidad'].loc[i] == 'Raleo':
...
# Agregar columna de inclinación donde se le asigna el valor 1 si hay torre y 0 en otro caso
inclinacion = []
for i in cosecha_dataframe.index:
...
# Exportan datos corregidos Cosecha

cosecha_dataframe.to_csv('./datasets/madera/datos_corregidos_cosecha.csv', sep=',', index=False)
