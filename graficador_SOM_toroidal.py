...
exec(open('./Resultados_SOM/resultadosSOMToroidal-05-100-8x8','r').read())
...
###########################################
### Grafico por atributos de toroide #####
...
#for num in range(10):
    ...
    ax1 = fig.add_subplot(8,3,1+num, projection='3d')
    
    ax1.set_title(nombres3[num], fontname="serif", fontsize = 4, pad = 1)
    ...
    radio = c
    n_puntos = 400
    y1 = np.linspace(-c, c, n_puntos)
    x1 = np.sqrt(np.power(radio, 2) - np.power(y1, 2))
    z1 = np.zeros(shape = (n_puntos))
    
    ax1.plot(x1,y1,z1, c = 'black', linewidth = 0.2)
    ax1.plot(-x1,y1,z1, c = 'black', linewidth = 0.2)
    
    for i in range(m1+1):
        ax1.plot(x[i,:],y[i,:],z[i,:], c = 'black', linewidth = 0.4)
    ...
    for i in range(m1):
        for j in range(n1):
            rgba = cmap(matriz_peso1[i, j])
            ax1.scatter(x[i][j], y[i][j], z[i][j], s = 0.3, color=rgba)
    #ax1.view_init(36, 26)
    
    #plt.savefig('./Resultados_SOM/resultadosSOMToroidal-'+str(l_rate)[2:]+'-'+str(+n_iter)+'-'+str(m1)+'x'+str(n1)+'-TOROIDE')
    
    #plt.show()
    
    #### FIN grafico toroidal ###
    #############################
...
#plt.savefig("lectorToroidal2.pdf", bbox_inches='tight')


### Fin Grafico por atributos de toroide #####
##############################################
...
###########################################
### Grafico por atributos de Panal #####
...
#fig.savefig("lector_toroidal_panal.pdf", bbox_inches='tight')


### Fin Grafico por atributos de Panal #####
##############################################
...
#plt.savefig('./Resultados_SOM/resultadosSOMToroidal-'+str(l_rate)[2:]+'-'+str(+n_iter)+'-'+str(m1)+'x'+str(n1)+'-LINEA')

plt.show()


peso_k = km[cluster_sel-1].labels_
peso_k1 = km[cluster_sel-1].labels_.reshape(m1,n1)
...
#plt.savefig("kmeansToroidal2d.pdf", bbox_inches='tight')
